...
void counterspell(Spell *spell) {
  ...
  if (Fireball *which_spell = dynamic_cast<Fireball *>(spell))
    which_spell->revealFirepower();

  else if (Frostbite *which_spell = dynamic_cast<Frostbite *>(spell))
    which_spell->revealFrostpower();

  else if (Thunderstorm *which_spell = dynamic_cast<Thunderstorm *>(spell))
    which_spell->revealThunderpower();

  else if (Waterbolt *which_spell = dynamic_cast<Waterbolt *>(spell))
    which_spell->revealWaterpower();

  else {
    auto name = spell->revealScrollName();
    auto journal = SpellJournal::read();

    vector<vector<size_t>> data(name.size() + 1,
                                vector<size_t>(journal.size() + 1, 0));
